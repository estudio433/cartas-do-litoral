    ...
    <div className="pt-32 pb-32 px-6 flex-1 bg-background">
      <div className="max-w-7xl mx-auto">
        <header className="mb-20 text-center max-w-2xl mx-auto">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-serif text-5xl mb-6">
            Diário do Litoral
          </motion.h1>
        ...
        <div className="grid md:grid-cols-3 gap-x-8 gap-y-16">
          {posts.map((post, i) => (
            <motion.article key={post.slug} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 + 0.2, duration: 0.6 }} className="group">
              <Link href={`/diario/${post.slug}`}>
                ...
                <div className="space-y-4">
                  ...
                  <h2 className="font-serif text-3xl group-hover:text-accent-warm transition-colors duration-300">{post.title}</h2>
                  <p className="text-muted-foreground/90 line-clamp-3 leading-relaxed">{post.excerpt}</p>
