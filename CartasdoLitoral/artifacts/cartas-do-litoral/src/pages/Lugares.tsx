...
const lugares = [
  {
    ...
    nome: "Ostradamus",
    local: "Rod. Baldicero Filomeno, 7640 — Ribeirão da Ilha",
...
export default function Lugares() {
  ...
  const filtered = active === "Todos"
    ? lugares
    : lugares.filter(l => l.tipo === active);
  ...
  return (
    <div className="pt-20">
        ...
        <div className="max-w-3xl mx-auto">
          <motion.div initial="hidden" animate="visible" variants={fadeIn}>
            ...
            <p className="text-muted-foreground text-lg leading-relaxed max-w-xl">
              Uma curadoria de lugares que ficaram. Sem estrelas, sem avaliações. Só o que vale a pena.
            </p>
            <p className="text-sm text-muted-foreground/50 italic mt-4 max-w-xl">
              Alguns vividos, alguns contados por quem os conhece de cor.
            </p>
          </motion.div>
        </div>
      </section>

      {/* Newest entry callout */}
      <section className="px-6 pb-10 bg-background">
        <div className="max-w-3xl mx-auto">
          <motion.div
            ...
            animate="visible"
            variants={fadeIn}
            className="border-l-2 border-accent-warm/40 pl-6 py-2"
          >
            <p className="text-xs uppercase tracking-widest text-accent-warm/70 font-semibold mb-1">
              Último lugar adicionado
            </p>
            <p className="font-serif text-lg text-foreground">
              {newestEntry.nome}
              <span className="font-sans text-sm text-muted-foreground/60 ml-3 font-normal tracking-wide">
                {newestEntry.cidade} · {newestEntry.tipo}
              </span>
            </p>
          </motion.div>
        </div>
      ...
      {/* Filters — only show categories that have entries */}
            ...
            {filtered.map((l, i) => (
              ...
              >
                ...
                <div className="shrink-0 text-right w-24 pt-1 space-y-1">
                  <span className="text-xs uppercase tracking-widest text-accent-warm font-semibold block">
                    {l.tipo}
                  </span>
                  <span className="text-xs text-muted-foreground/50 tracking-wide block">
                    {l.cidade}
                  </span>
                </div>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-4 mb-1">
                    ...
                    {l.isNew && (
                      <span className="text-[10px] uppercase tracking-widest text-accent-warm/70 border border-accent-warm/30 px-2 py-0.5 shrink-0 mt-1">
                        Novo
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground/60 mb-4 tracking-wide">
                    {l.local}
                  </p>
                  <p className="text-muted-foreground leading-relaxed">{l.texto}</p>
                  {l.source && (
                    <p className="text-xs text-muted-foreground/50 italic mt-4 tracking-wide">
                      — {l.source}
                    </p>
                  )}
                </div>
